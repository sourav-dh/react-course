import RestCardComponent, {
	LabeledRestCardComponent,
} from "./RestCardComponent";
...
const Body = () => {
	...
	const LabeledRestCard = LabeledRestCardComponent(RestCardComponent);
	...
	return listOfRes.length === 0 ? (
		...
		<div className="res-body">
			<div className="res-search">
				...
				<button
					onClick={() => {
						const searchedRests = listOfRes.filter((val) =>
			...
			<div className="res-card-container">
				{filteredList.map((val) => (
					<Link to={"/restaurant/" + val.info.id} key={val.info.id}>
						{val.info.avgRating >= 4.5 ? (
							<LabeledRestCard info={val.info} />
						) : (
							<RestCardComponent info={val.info} />
						)}
					</Link>
				))}
			</div>
